...
// +----------------------------------------------------------------------+
//
// $Id: Lexer.php,v 1.4 2006/05/03 01:26:48 sjhannah Exp $
//
    ...
    while (true){
        ...
        switch($state) {
            ...
            case 1:
                $c = $this->get();
               	if ( $c == '`' and $quote == '`' ){
               		// this may be the end, but if the quote is followed by '.', then we want the entire
               		// identifier.
               		$nex = $this->get();
               		if ( $nex == '.' ){
               			$nex2 = $this->get();
            ...
            case 2:
            	if ( $quote == '`' and $c == '`'){
            		$this->tokText = substr($this->string, $this->tokStart+1, $this->tokLen-2);
            	} else {
            		$this->unget();
                ...
                $this->tokText = str_replace('`','',$this->tokText);
                
                $testToken = strtolower($this->tokText);
                if (!($quote == '`' and $c == '`') and isset($this->symbols[$testToken])) {
