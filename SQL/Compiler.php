...
// }}}
	...
	function compileFunction($arg, $writeAlias = true){
		if ( !isset( $arg['name']) ){
			$err = self::raiseError("Expected function name.");
    ...
    function getWhereValue ($arg)
    {
        switch ($arg['type']) {
            ...
            case 'subquery':
				...
				if ( self::isError($value) ) return $value;
				$value = '('.$value.')';
				unset($subCompiler);
			...
			for ($i = 0; $i < sizeof ($arg['type']); $i++) {
				switch ($arg['type'][$i]) {
					...
					case 'function':
						...
						$value[] = $val;
					default:
						return self::raiseError('Unknown type: '.$arg['type'][$i]);
				}
			}
		} else {
        ...
        if ( $this->version <= 1 ){
			...
			for ($i = 0; $i < sizeof ($this->tree['columns']); $i++) {
				switch ( $this->tree['columns'][$i]['type'] ){
					...
					default:
						return self::raiseError("Unexpected column type '".$this->tree['columns'][$i]['type']);
...
//	{{{ function newInstance
	 ...
	 */
	function &newInstance($type=null){
		if ( $type === null ){
			...
			if ( !preg_match('/^[0-9a-z_]+$/', $type) ){
				return self::raiseError('Invalid type name specified: '.$type);
			}
			...
			if ( !class_exists($class_name) ){
				if ( file_exists( dirname(__FILE__).'/Compiler/'.$type.'.php') ){
					//echo "File exists";
					require_once 'Compiler/'.$type.'.php';
				}
			}
			if ( !class_exists($class_name) ){
				 return self::raiseError('Could not create compiler of type '.$type.' because no corresponding class exists in the SQL/Compiler directory');
			}
